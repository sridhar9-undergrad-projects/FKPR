...
for dirs in os.listdir(Constants.TRAIN_DATA_PATH):
    ...
    for file in os.listdir(path):
        ...
        _,surfdes=DetectIP.find_SURF_Descriptors(image)
        _,siftdes=DetectIP.find_SIFT_Descriptors(image)
        trainfeatures[classes].append((surfdes,siftdes))
    classes+=1
...
for dirs in os.listdir(Constants.TEST_DATA_PATH):
    ...
    for file in os.listdir(path):
        image =  cv2.imread(os.path.join(path,file),0);
        _,test_img_ip_SURF=DetectIP.find_SURF_Descriptors(image)
        _,test_img_ip_SIFT=DetectIP.find_SIFT_Descriptors(image)
        ...
        for train_class in range(totalclasses):
            ...
            for img in range(totalImages):
                train_img_ip_SIFT=trainfeatures[train_class][img][1]
                train_img_ip_SURF=trainfeatures[train_class][img][0]
                matching_score_SIFT=CalcMatchingScore.matchingScore(test_img_ip_SIFT,train_img_ip_SIFT)
                matching_score_SURF=CalcMatchingScore.matchingScore(test_img_ip_SURF,train_img_ip_SURF)
                temp_matching_score = float(matching_score_SIFT)*Constants.SIFT_WEIGHT + float(matching_score_SURF)*Constants.SURF_UPRIGHT
        ...
        if(classified_class == classes):
            correct_matches+=1
            class_matching+=1
        saveFile.write(str(classified_class)+" "+str(classes)+" "+str(matching_score)+'\n')
